...
export default function CopyStatsCurlButton({ apiBase, status, severity, bbox, since }: Props) {
  const compute = React.useCallback(() => {
    ...
    const url = `${apiBase}/incidents/stats${qs.toString() ? `?${qs.toString()}` : ''}`;
    return `curl -fsSL ${JSON.stringify(url)}`;
  }, [apiBase, status, severity, bbox, since]);

  return <CopyButton value={compute()} label="Copy curl (stats)" />;
}
